  ...
  Console(int width, int height, [Font font]) : super(width, height) {
    ...
    this.font.loaded.then((_) {
      _foreCanvas
          ...
          ..style.left = '0'
          ..style.margin = '0'
          ..width = cellData.width * this.font.char_width
          ..height = cellData.height * this.font.char_height;
      _backCanvas
          ..style.position = 'absolute'
          ..style.top = '0'
          ..style.left = '0'
  ...
  flush() {
    font.loaded.then((_) {
      ...
      window.requestAnimationFrame((frame) {
        ...
        for (int x = cellData.width - 1; x >= 0; x--) {
          for (int y = cellData.height - 1; y >= 0; y--) {

            // prepare foreground
            font.chars[cellData.get(x, y).glyph].context2D
                ..fillStyle = cellData.get(x, y).foreColor.toString()
                ..globalCompositeOperation = 'source-in'
                ..fillRect(0, 0, font.char_width, font.char_height);

            // print background but only if there was a change
            _backCanvas.context2D
                ..fillStyle = cellData.get(x, y).backColor.toString()
            ...
            _foreCanvas.context2D
                ...
                ..drawImageScaled(
                    font.chars[cellData.get(x, y).glyph],
                    x * font.char_width,
                    y * font.char_height,
                    font.char_width,
                    font.char_height);
